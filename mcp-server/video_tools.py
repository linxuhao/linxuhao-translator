"""The VideoMCP tool surface. One definition, used by both deployments.

Transport is chosen by GPU_SERVICE_URL: unset means the backend is on this machine and the
unix socket is used; set means this runs in the MCP layer and every call is an authenticated
POST to the GPU-side facade. Nothing above this line differs between the two, which is the
point — the hand-copied second implementation had already drifted a whole feature behind.
"""
import asyncio
import json
import os
...
# Set only where this runs apart from the backend. Same name and same credential file the
# rest of the MCP layer already uses for the scheduler, so there is one convention, not two.
GPU_RPC=os.getenv('GPU_SERVICE_URL','').rstrip('/')
GPU_TOKEN_FILE=os.getenv('GPU_TOKEN_FILE','/run/secrets/GPU_API_TOKEN')
...
async def call(action,**arguments):
    ...
    if GPU_RPC:
        ...
        token=Path(GPU_TOKEN_FILE).read_text().strip()
        async with httpx.AsyncClient(timeout=930) as client:
            response=await client.post(GPU_RPC+'/rpc',json=payload,headers={'Authorization':'Bearer '+token})
    else:
