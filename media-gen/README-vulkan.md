...
和 actor 同一个病、同一个解法的视觉版。`generate_image` 每次给的是"长得不一样的人" ——
同一个角色的头像 / 战斗立绘 / 地图小人，直接用文字描述生成出来是三个人。

`create_character` 定妆一张参考图存进 `/characters/<name>.png`，之后 `character_image`
把它作为 `ref_images` 走图生图，长相与场景描述解耦。

验收用 GPU0 上的 VLM 当判官（和用 ASR 判配音一个道理），同一组场景做有/无参考图对照，
每边判三次：

| | 三次评分 | 中位数 |
|---|---|---|
| 带参考图（character） | 9 / 9 / 9 | **9** |
| 无参考图（长相只写进 prompt） | 7 / 6 / 9 | 7 |

注意带参考图那组三次全是 9，判官没有分歧；对照组 6~9 摇摆，其中一次的评语是
"look like different individuals"。人眼复核确认"同一个人，不同的风格都对得上"。

定妆图同样**必须先看**，理由和参考音一样：定砸了会把整个角色锁死在错的长相上，
而且之后每一张都错得很一致。`/characters` 是独立的卷，同样不进仓库、同样不可复现。
